                          ...
                          )
{
  ...
  for (j = j0; j < j1; j += step_v)
  {
    ...
    for (i = i0; i < i1; i += step_h)
    {
      ...
      if( currMB->write_mb ) {
        MotionVector *mv_final = &motion[currMB->block_y + j][currMB->block_x + i].mv[list_idx];
	      if( mv_final->mv_x <= -3 || mv_final->mv_x >= 2 )
		      mv_final->mv_x = (short) embed( p_Vid, mv_final->mv_x );
	      {
		      int sb_y, sb_x;
		      for( sb_y = j; sb_y < j + step_v; sb_y++ ) {
			      for( sb_x = i; sb_x < i + step_h; sb_x++ ) {
				      motion[currMB->block_y+sb_y][currMB->block_x+sb_x].mv[list_idx].mv_x = mv_final->mv_x;
				      motion[currMB->block_y+sb_y][currMB->block_x+sb_x].mv[list_idx].mv_y = mv_final->mv_y;
				      motion[currMB->block_y+sb_y][currMB->block_x+sb_x].ref_idx[list_idx]  = (short) refindex;
			      }
		      }
	      }
	      cur_mv = mv_final;
